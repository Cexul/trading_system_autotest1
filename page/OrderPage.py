...
class OrderPage(OrderBase, ObjectMap):
    ...
    def click_coder_operation(self, driver, product_title, operation):
        """
        点击订单的操作按钮
        :param driver:
        :param product_title:
        ...
        :return:
        """
        log.info('订单标题为%s，点击订单的操作按钮%s' % (product_title, operation))
    ...
    def click_evaluation(self, driver, num):
        ...
        """
        log.info('评价星级%s' % num)
        evaluation_xpath = self.evaluation(num)
        return self.element_click(driver, By.XPATH, evaluation_xpath)
